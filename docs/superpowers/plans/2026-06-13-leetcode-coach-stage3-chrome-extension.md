...
- **The one unverifiable item** (the exact `/check/` result endpoint, since run/submit is login-gated) is isolated to a single regex `shouldCaptureUrl`, documented as such, and has an explicit confirm-against-live step in Task 10 — no guessing shipped silently.
